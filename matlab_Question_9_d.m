...
% Define the bandwidth for the band-stop filter
lower_cutoff_frequency = 20; % Adjust as needed
upper_cutoff_frequency = 40; % Adjust as needed
% Define a band-stop filter in the frequency domain
[M, N] = size(F);
u = 0:(M-1);
v = 0:(N-1);
idx = find(u > M/2);
...
[V, U] = meshgrid(v, u);
D = sqrt(U.^2 + V.^2);
band_stop_filter = double((D < lower_cutoff_frequency) | (D > upper_cutoff_frequency));
% Apply the band-stop filter to the frequency domain representation of the image
G = F_shifted .* band_stop_filter;
